...
meta_peso = round(float(df_cadastro[df_cadastro['CPF']== cpf]['META_PESO'].iloc[0]), 2)
peso_inicial = round(float(df_usuario['Peso'].iloc[0]), 2)
meta_gordura = round(float(df_cadastro[df_cadastro['CPF']== cpf]['META_GORDURA'].iloc[0]), 2)
gordura_inicial = round(float(df_usuario['Percentual de Gordura Corporal'].iloc[0]), 2)
meta_musculo = round(float(df_cadastro[df_cadastro['CPF']== cpf]['META_MUSCULO'].iloc[0]), 2)
musculo_inicial = round(float(df_usuario['Percentual de Musculatura Corporal'].iloc[0]), 2)
meta_visceral = round(float(df_cadastro[df_cadastro['CPF']== cpf]['META_VISERAL'].iloc[0]), 2)
gordura_visceral_inicial = round(float(df_usuario['Gordura Visceral'].iloc[0]), 2)

for item in lista_selecionada:
    if item == 'Peso':
        delta_peso = peso_inicial - meta_peso
        percentual_meta_peso = round((peso_inicial - df_usuario['Peso'].iloc[-1]) / delta_peso * 100, 2)
...
div_int = len(lista_selecionada) // 3
for n in range(div_int):    
    col1.metric(label=lista_selecionada[n], 
                value=df_usuario[lista_selecionada[n]].iloc[-1], 
                delta=round(float(df_usuario[lista_selecionada[n]].iloc[-1] - df_usuario[lista_selecionada[n]].iloc[0]), 2))
    col2.metric(label=lista_selecionada[n+1], 
                value=df_usuario[lista_selecionada[n+1]].iloc[-1], 
                delta=round(float(df_usuario[lista_selecionada[n+1]].iloc[-1] - df_usuario[lista_selecionada[n+1]].iloc[0]), 2))
    col3.metric(label=lista_selecionada[n+2], 
                value=df_usuario[lista_selecionada[n+2]].iloc[-1], 
                delta=round(float(df_usuario[lista_selecionada[n+2]].iloc[-1] - df_usuario[lista_selecionada[n+2]].iloc[0]), 2))
if len(lista_selecionada)%3 == 1:
    col2.metric(label=lista_selecionada[-1],
                value=df_usuario[lista_selecionada[-1]].iloc[-1],
                delta=round(float(df_usuario[lista_selecionada[-1]].iloc[-1] - df_usuario[lista_selecionada[-1]].iloc[0]), 2))
